<?php
    ...
    include_once '../config.php';
        ...
        if(empty($search_value)){
            $search_query = "SELECT * FROM subsidy ";
        }
        else{
            $search_query = "SELECT * FROM subsidy WHERE subname LIKE '%$search_value%' ";
        }
        ...
        if(mysqli_num_rows($search_query_run) > 0){
            while($row = mysqli_fetch_assoc($search_query_run))
            {
                $output .= '<div class="customrow">
                                ...
                                <div class="more">
                                    <a href="subsidy-detail.php?subid='.$row['subid'].'"> <button type="button" class="btn btn-outline-warning w-100 "> Explore
                                        </button></a>
                                </div>
                            </div>';
            }
